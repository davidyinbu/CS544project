...
plot_ly(data = pie_data, labels = ~Brand, values=~Freq,type="pie")%>%
  layout(title = "TOP10 popular car")
...
V_car$model_name <- as.factor(V_car$model_name)
...
Top_5_v_dataset%>%group_by(model_name)%>%
  do(plot_ly(., y = ~model_name, x = ~price_usd)) %>%
  add_histogram(color = ~model_name)
...
#
Top_5_v_dataset%>%group_by(model_name,year_produced)
